...
+ $1/(n ln n)$、$1/(n (ln n)^p)$ 型：优先积分判别或 Cauchy 凝聚判别。
+ $(-1)^n b_n$ 型：优先绝对收敛，再考虑 Leibniz判别。
+ 可拆成
  $
    a_n = b_n - b_(n + 1)
  $
...
若 $a_n$ 自身带有振荡，但部分和 $A_n = sum_(k = 1)^n a_k$ 始终有界，那么这些振荡不会无限累积；这时只要再乘上一个单调趋于 $0$ 的 $b_n$，整体就会被逐步压住，这就是 Dirichlet判别法。

而 Abel判别法则是另一种稳定机制：若 $sum a_n$ 本身已经收敛，再乘上一个单调有界的 $b_n$，这种乘法不会破坏原有的收敛性。因此可以把 Dirichlet理解为“有界振荡 + 衰减因子”，把 Abel理解为“已收敛级数 + 温和权重”。
...
#strategy[正项级数的判别顺序][
  ...
  + 若通项里有阶乘、指数，或容易写出 $a_(n + 1) / a_n$，优先用 d'Alembert判别法。
  + 若通项整体被某个 $n$ 次方包住，优先看 $root(n, a_n)$，即 Cauchy判别法。
  + 若比值极限恰好等于 $1$，再看它是否有
    $
      a_(n + 1) / a_n = 1 - p / n + o(1 / n)
    $
    这一类更细的展开，此时优先用 Raabe判别法。
...
这里的 Cauchy判别法指根值判别法，它考察的是
$
  root(n, a_n)
$
所代表的“平均指数增长率”。因此当通项形如 $b_n^n$，或包含 $n$ 次幂、$n^2$ 次幂时，它往往比 d'Alembert判别法更直接。不要把这里的 Cauchy判别法与前面的 Cauchy凝聚判别法混淆。
...
当 d'Alembert判别法得到的极限恰好是 $1$ 时，只知道相邻两项“差不多一样大”，还不足以判断级数敛散。Raabe判别法进一步比较这种“差不多”到底精确到什么程度。

若
$
  a_(n + 1) / a_n = 1 - p / n + o(1 / n)
$
那么它的临界行为就和 $1 / n^p$ 十分接近，所以门槛正好落在 $p = 1$。因此 Raabe判别法可以看作 d'Alembert判别法在极限等于 $1$ 时的细化版本。
...
#example[积分判别法的标准模型][
  ...
  这说明：含有对数修正的正项级数，往往不是 d'Alembert判别法或 Cauchy判别法最擅长处理的对象，而更适合用积分判别法。
]

#example[几类典型通项如何选判别法][
  + $sum n! / 3^n$：阶乘与指数并存，优先用 d'Alembert判别法。
  + $sum ((n - 1) / n)^(n^2)$：通项带有高次幂，优先用 Cauchy判别法。
